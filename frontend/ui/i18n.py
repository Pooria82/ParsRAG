...
WELCOME_MARKDOWN: Final[dict[str, str]] = {
    LANG_FA: (
        ...
        "- برای تغییر حالت، زبان (فارسی/انگلیسی) و سایر پارامترها می‌توانید از دکمه انتخاب‌گر حالت در نوار پایین یا منوی تنظیمات (⚙️) استفاده فرمایید.\n\n"
    ...
    LANG_EN: (
        ...
        "- Switch modes, language (English/Persian), or operational thresholds anytime using the mode pill in the bottom composer or the Settings drawer (⚙️).\n\n"
...
def format_file_limit_exceeded(
    ...
    if norm_lang == LANG_EN:
        return (
            f"⚠️ **Document Limit Exceeded:** A maximum of {max_limit} files is allowed per session.\n"
...
def format_ingest_progress(lang: str, count: int) -> str:
    ...
    norm_lang = normalize_language(lang)
    if norm_lang == LANG_EN:
        return f"⏳ Processing and indexing {count} document(s) in local vector database..."
